...
class MapCanvas(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.lidar_points: set[tuple[float, float]] = set()
    ...
    def _draw_grid(self, p):
        ...
        p.setPen(QPen(QColor(255, 255, 255, 30), 1, Qt.DotLine))
        ...
        p.setPen(QPen(QColor(220, 50, 50, 140), 1.5))  
        p.drawLine(0, int(origin.y()), W, int(origin.y()))
        p.setPen(QPen(QColor(50, 220, 80, 140), 1.5))  
    ...
    def _draw_lidar(self, p):
        ...
        for wx, wy in list(self.lidar_points):

            cp = self._w2c(wx, wy)

            if (
                -10 <= cp.x() <= self.width() + 10
                and
                -10 <= cp.y() <= self.height() + 10
            ):

                # Bigger points
                p.drawEllipse(cp, 2.5, 2.5)

    def _draw_trail(self, p):
        if len(self.trail) < 2: return
        n   = len(self.trail)
        pen = QPen(QColor(59, 130, 246, 180), 2, Qt.SolidLine)
        ...
        for i in range(1, n):
            ...
            pen.setColor(QColor(59, 130, 246, alpha))
            p.setPen(pen)
    ...
    def _draw_bot(self, p):
        ...
        glow.setColorAt(0, QColor(34, 197, 94, 60))
        ...
        p.setPen(Qt.NoPen)
        p.drawPolygon(QPolygonF([QPointF(bw/2+14,-4), QPointF(bw/2+20, 0), QPointF(bw/2+14,  4)]))
        p.restore()
...
class TelemetryPanel(QFrame):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("""
            QFrame { background:#111827; border:1px solid #374151; border-radius:10px; }
            QLabel#title { color:#60a5fa; font-family:'Segoe UI'; font-size:18px; font-weight:700; border:none; }
            QLabel#label { color:#d1d5db; font-family:'Segoe UI'; font-size:14px; font-weight:600; letter-spacing:1px; border:none; }
            QLabel#value { color:#22c55e; font-family:'Consolas'; font-size:22px; font-weight:700; min-width:130px; background:#0b1220; border:1px solid #374151; border-radius:6px; padding:6px 14px; }
            QLabel#unit { color:#9ca3af; font-family:'Segoe UI'; font-size:13px; font-weight:600; min-width:38px; border:none; }
            QWidget#row { background:#0f172a; border:1px solid #1f2937; border-radius:8px; }
        """)
...
class SpeedValueDisplay(QFrame):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("QFrame { background:qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #180028, stop:1 #2b0040); border:2px solid #9333ea; border-radius:16px; }")
        ...
        top.setStyleSheet("color:#d8b4fe; font-family:'Segoe UI'; font-size:11px; font-weight:700; border:none;")
        lay.addWidget(top)

        self._val_lbl = QLabel("100")
        self._val_lbl.setAlignment(Qt.AlignCenter)
        self._val_lbl.setStyleSheet("color:#f5d0fe; font-family:'JetBrains Mono'; font-size:42px; font-weight:900; letter-spacing:2px; border:none;")
        lay.addWidget(self._val_lbl)

        bot = QLabel("%")
        bot.setAlignment(Qt.AlignCenter)
        bot.setStyleSheet("color:#c084fc; font-family:'Segoe UI'; font-size:11px; font-weight:700; border:none;")
        lay.addWidget(bot)
...
class ControlPad(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        left_panel = QWidget(); left_panel.setLayout(dpad)
        top_body.addWidget(left_panel)

        div = QFrame(); div.setFrameShape(QFrame.VLine)
        div.setStyleSheet("color:#30363d; background:#30363d;"); div.setFixedWidth(1)
        ...
        self.btn_stop.setStyleSheet("""
            QPushButton { background:#1a0700; color:#ff9800; border:2px solid #ff6f00; border-radius:8px; font-family:'Courier New'; font-size:16px; font-weight:bold; }
        ...
        sep.setStyleSheet("color:#1e2d3d; background:#1e2d3d;"); sep.setFixedHeight(1)
        outer.addWidget(sep)

        BH = 46   
        def _gb(text, bg, hover, color="white"):
            b = QPushButton(text); b.setFixedHeight(BH); b.setFocusPolicy(Qt.NoFocus)
            b.setStyleSheet(f"QPushButton {{ background:{bg}; color:{color}; border:2px solid {hover}; border-radius:9px; font-family:'Segoe UI'; font-size:12px; font-weight:700; }} QPushButton:hover {{ background:{hover}; color:white; }} QPushButton:pressed {{ background:#000; }}")
            return b
        ...
        def leg_lbl(txt, clr):
            l = QLabel(txt); l.setStyleSheet(f"color:{clr}; font-family:'Courier New'; font-size:9px;")
            return l

        lg = QHBoxLayout()
        lg.addWidget(leg_lbl("🔴 MOVE  🟡 HOME", "#ff7070"))
        lg.addWidget(leg_lbl("🔵 SPD ±5", "#00bcd4")); lg.addStretch()
        outer.addLayout(lg)
...
class EncoderGraphDialog(QDialog):
    def __init__(self, parent=None):
        ...
        for clr, txt in [("#00c853", "━━  LEFT MOTOR"), ("#2979ff", "━━  RIGHT MOTOR")]:
            l = QLabel(txt); l.setStyleSheet(f"color:{clr}; font-family:'Courier New'; font-size:10px; font-weight:bold; padding-right:20px;")
            leg.addWidget(l)
        leg.addStretch()
        ...
        self.ax.xaxis.label.set_color("#8b949e"); self.ax.yaxis.label.set_color("#8b949e")
        self.ax.set_xlabel("Distance Moved (m)"); self.ax.set_ylabel("Encoder Value (m)")
        ...
        for w in [self._lbl_l, self._lbl_r, self._lbl_d]: vrow.addWidget(w)
        vrow.addStretch()
    ...
    def _val_box(self, name, color):
        ...
        w.setStyleSheet(f"color:{color}; font-family:'Courier New'; font-size:10px; font-weight:bold; background:#161b22; border:1px solid #30363d; border-radius:6px; padding:5px 14px;")
        return w
...
# ─────────────────────────────────────────────────────────────────────────────
#  ROS2 Node Wrapper Node Pipeline
# ─────────────────────────────────────────────────────────────────────────────
class RealOdomSubscriber:
    ...
    def send_cmd(self, linear_x, angular_z, command=""):
        if command: self.last_command = command
        if self._node: self._node.send_cmd(linear_x, angular_z)

    def shutdown(self):
        if self._node:
            self._node.destroy_node()
            rclpy.shutdown()
        if hasattr(self, "_log"): self._log.close()
...
class ControlButton(QPushButton):
    ...
    def __init__(self, label, color="red", parent=None, size=52, font_size=None):
        ...
        self.setStyleSheet(f"QPushButton {{ background:{base}; border-radius:12px; color:white; font-size:{fs}px; font-weight:bold; border:2px solid {hover}; }} QPushButton:hover {{ background:{hover}; }} QPushButton:pressed {{ background:white; color:{base}; }}")
...
class _OdomNode(Node):
    def __init__(self, subscriber):
        ...
        self.create_subscription(LaserScan, "/scan", self._laser_cb, qos_profile)

        # 👑 Add these position trackers for incremental distance gates:
        self._last_map_x = None
        self._last_map_y = None
        self._last_map_angle = None
    ...
    def _laser_cb(self, msg: LaserScan):
        mw = self.sub_ref.mw
        bx, by, b_angle_deg = mw.bot_x, mw.bot_y, mw.bot_angle
        
        # 👑 DISTANCE & ROTATION GATE:
        # Initialize trackers if this is the first packet received
        if self._last_map_x is None:
            self._last_map_x = bx
            self._last_map_y = by
            self._last_map_angle = b_angle_deg

        # Compute physical translation distance and rotation step changes
        travel_dist = math.hypot(bx - self._last_map_x, by - self._last_map_y)
        angle_diff = abs(b_angle_deg - self._last_map_angle)
        while angle_diff > 180: angle_diff -= 360
        angle_diff = abs(angle_diff)

        # Skip updates unless robot moved enough
        if travel_dist < 0.12 and angle_diff < 6.0:
            return
        # Update baseline markers once travel threshold is crossed
        self._last_map_x = bx
        self._last_map_y = by
        self._last_map_angle = b_angle_deg
            
        b_angle_rad = math.radians(b_angle_deg) 
        global_map = set(mw.map_canvas.lidar_points)
        angle = msg.angle_min
        
        # 👑 ROTATION FIX: Changed from -90 to 0 degrees to correct the 90° layout rotation
        sensor_offset = math.radians(0)
        ...
        for r in msg.ranges:
            ...
            if 0.45 <= r <= 5.0:
                corrected_angle = angle + sensor_offset
                
                lx_local = r * math.cos(corrected_angle)
                ly_local = r * math.sin(corrected_angle)
                
                wx = bx + (lx_local * math.cos(b_angle_rad) - ly_local * math.sin(b_angle_rad))
                wy = by + (lx_local * math.sin(b_angle_rad) + ly_local * math.cos(b_angle_rad))
                
                # Deduplicate points inside a 2cm grid array
                grid_x = round(wx * 10) / 10
                grid_y = round(wy * 10) / 10
                
                # ===== OCCUPANCY DEDUP FILTER =====

                skip_point = False

                # Check nearby existing points
                for ex, ey in global_map:

                    # Distance from existing scanned point
                    d = math.hypot(grid_x - ex, grid_y - ey)

                    # If already scanned nearby, skip
                    if d < 0.08:
                        skip_point = True
                        break

                # Add only fresh points
                if not skip_point:
                    mw.map_canvas.lidar_points.add((grid_x, grid_y)) 
    ...
    def _cb(self, msg):
        ...
        # 👑 FIX 1: Extract velocity markers accurately from twist frame
        ...
        # 👑 FIX 2: Correct standard differential geometry integration math
        self.sub_ref.left_enc  += (v_linear - w_angular * half_wheel_base) * dt
        self.sub_ref.right_enc += (v_linear + w_angular * half_wheel_base) * dt

        # 👑 FIX 3: Compute clean, inverted canvas yaw coordinates 
        q = msg.pose.pose.orientation
        yaw = -math.degrees(math.atan2(2*(q.w*q.z + q.x*q.y), 1 - 2*(q.y*q.y + q.z*q.z)))
        
        mw.current_speed = abs(v_linear)
        mw.current_ang_speed = abs(math.degrees(w_angular))

        # ===== DEAD RECKONING INTEGRATION =====

        if not hasattr(self, "_pose_initialized"):
            self._pose_initialized = True
            self._last_stamp_ns_pose = now_ns
            return

        dt_pose = max(
            0.001,
            min(0.1, (now_ns - self._last_stamp_ns_pose) / 1e9)
        )
        ...
        alpha_yaw = 0.25
        mw.bot_angle = (
            (1 - alpha_yaw) * mw.bot_angle
            + alpha_yaw * yaw
        )

        # Normalize angle
        while mw.bot_angle > 180:
            mw.bot_angle -= 360
        ...
        heading_rad = math.radians(mw.bot_angle)

        mw.bot_x += v_linear * math.cos(heading_rad) * dt_pose
        mw.bot_y += v_linear * math.sin(heading_rad) * dt_pose

        # Total travel distance
        self.sub_ref.total_dist += abs(v_linear) * dt_pose
        mw.map_canvas.update_bot(
            mw.bot_x,
            mw.bot_y,
            mw.bot_angle
        )
        ...
        self.sub_ref._csv.writerow([ts, f" {self.sub_ref.last_command}", f" {round(self.sub_ref.total_dist, 4)}", f" {round(self.sub_ref.left_enc, 4)}", f" {round(self.sub_ref.right_enc, 4)}"])
...
class AMRMainWindow(QMainWindow):
    def __init__(self):
        ...
        def _zb(label, tip, color_pair, slot, w=36, h=28):
            bg, hv = color_pair
            b = QPushButton(label); b.setFixedSize(w, h); b.setToolTip(tip); b.setFocusPolicy(Qt.NoFocus)
            b.setStyleSheet(f"QPushButton {{ background:{bg}; color:white; border:1px solid {hv}; border-radius:6px; font-family:'Courier New'; font-size:11px; font-weight:bold; }} QPushButton:hover {{ background:{hv}; }} QPushButton:pressed {{ background:white; color:{bg}; }}")
        ...
        for w in [self._btn_zoom_in, self._btn_zoom_out, self._btn_zoom_rst, pan_hint]: ztb.addWidget(w)
        lv.addLayout(ztb)
        ...
        self.timer = QTimer()
        self.timer.timeout.connect(self.map_canvas.update)
        self.timer.start(20)
        self.setFocusPolicy(Qt.StrongFocus); self.setFocus()

    def _show_home_prompt(self):
        self.control_pad.btn_set_home.setStyleSheet("QPushButton { background:#f9a825; border-radius:8px; color:#000; font-size:20px; font-weight:bold; border:3px solid #fff176; } QPushButton:hover { background:#fdd835; }")
    ...
    def _set_home(self):
        ...
        self.control_pad.btn_set_home.setStyleSheet(f"QPushButton {{ background:{base}; border-radius:8px; color:white; font-size:20px; font-weight:bold; border:2px solid {hover}; }} QPushButton:hover {{ background:{hover}; }} QPushButton:pressed {{ background:white; color:{base}; }}")
    ...
    def _stop_and_fit_path(self):
        ...
        self._following_waypoints = False; self._returning_home = False; self._wp_mode_active = False
    ...
    def _on_waypoint_added(self, wx, wy):
        self.control_pad.btn_waypoint.setText(f"WP: {len(self.map_canvas.waypoints)} set")
        self._following_waypoints = True
    ...
    def _save_waypoints(self):
        ...
        if not wps:
            ...
            return
        path, _ = QFileDialog.getSaveFileName(self, "Save Waypoints", os.getcwd(), "JSON Files (*.json)")
        if not path: return
        data = {"saved_at": datetime.datetime.now().isoformat(), "home": {"x": self.map_canvas.home_x, "y": self.map_canvas.home_y}, "waypoints": [{"x": wx, "y": wy} for wx, wy in wps]}
        with open(path, "w") as f: json.dump(data, f, indent=2)
        self.control_pad.btn_save_wp.setText("✓ Saved!")
        QTimer.singleShot(2000, lambda: self.control_pad.btn_save_wp.setText("💾 Save Waypoint"))

    def _load_waypoints(self):
        if not self._home_set: return
        path, _ = QFileDialog.getOpenFileName(self, "Load Waypoints", os.getcwd(), "JSON Files (*.json)")
        if not path: return
        try:
    ...
    def _show_enc_graph(self):
        if self._enc_dlg.isVisible(): self._enc_dlg.raise_(); self._enc_dlg.activateWindow()
        else: self._enc_dlg.show()
    ...
    def _set_move(self, direction, state):
        if not self._hard_stop and self._home_set: self._moving[direction] = state

    def _timed_move_1m(self, direction: str):
        if self._hard_stop or not self._home_set: return
    ...
    def _navigate_to(self, target_x, target_y, dt):
        dx, dy = target_x - self.bot_x, target_y - self.bot_y
        dist = math.hypot(dx, dy)
        if dist < WAYPOINT_REACH_DIST: return 0.0, 0.0, True
        ...
        w_val = max(-ang_max, min(ang_max, 2.5 * math.radians(angle_diff)))
        if abs(angle_diff) > 3.0: return 0.0, w_val, False
        return lin_max * min(1.0, dist / 0.4), w_val * 0.1, False

    def _smooth_wp_vel(self, current, target, ramp):
        diff = target - current
        return target if abs(diff) <= ramp else current + math.copysign(ramp, diff)
    ...
    def _tick(self):
        ...
        if not self._hard_stop and self._home_set:
            ...
            if self._returning_home:
                ...
                waypoint_info = "RTH"; _cmd_label = "RTH"
                ...
                if reached:
                    self._returning_home = False; self._ros.send_cmd(0.0, 0.0, "Stop")
                ...
                if idx < len(mc.waypoints):
                    ...
                    waypoint_info = f"{idx+1}/{len(mc.waypoints)}"; _cmd_label = f"WP{idx+1}"
                    self._wp_smooth_v = self._smooth_wp_vel(self._wp_smooth_v, v_nav, self._WP_LIN_RAMP)
                    self._wp_smooth_w = self._smooth_wp_vel(self._wp_smooth_w, w_nav, self._WP_ANG_RAMP)
                    ...
                    if reached:
                        ...
                        if mc.current_waypoint_idx >= len(mc.waypoints): self._following_waypoints = False
            else:
                lin = BASE_LINEAR_SPEED  * self.lin_speed_mod / 100.0
                ang = BASE_ANGULAR_SPEED * self.trn_speed_mod / 100.0
                
                if self._moving.get("fwd"):   v += lin  
                if self._moving.get("bwd"):   v -= lin  
                if self._moving.get("left"):  w += ang  
                if self._moving.get("right"): w -= ang  
                parts = [k for k, val in self._moving.items() if val]
                _cmd_label = "+".join(parts) if parts else "Stop"

        if (v != 0.0 or w != 0.0) and not self._hard_stop:
            self._ros.send_cmd(v, w, _cmd_label)
            self._last_sent_v, self._last_sent_w = v, w
        else:
            if not self._hard_stop and (self._last_sent_v != 0.0 or self._last_sent_w != 0.0):
                self._ros.send_cmd(0.0, 0.0, "Stop")
                self._last_sent_v = 0.0; self._last_sent_w = 0.0
        ...
        if not HAS_ROS and not self._hard_stop and self._home_set:
            ...
            self.current_speed, self.current_ang_speed = abs(v), abs(math.degrees(w))
            if self.map_canvas.mode == MapCanvas.MODE_FOLLOW:
                self.map_canvas.update_bot(self.bot_x, self.bot_y, self.bot_angle)

        self.control_pad.speed_display.set_value(self.lin_speed_mod)
        dx, dy = self.bot_x - self.map_canvas.home_x, self.bot_y - self.map_canvas.home_y
        self.telemetry.update_telemetry(dx, dy, math.degrees(math.atan2(dy, dx)) % 360, self.current_speed, self.current_ang_speed, self.lin_speed_mod, self.trn_speed_mod, waypoint_info)
    ...
    def closeEvent(self, e):
        self._ros.shutdown(); self._enc_dlg.close(); super().closeEvent(e)
